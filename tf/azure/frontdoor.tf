...
resource "azurerm_cdn_frontdoor_origin" "geofoodtruck_az_cdn_frontdoor_data_sforg_origin" {
  ...
  host_name                       = "data.sfgov.org"
  origin_host_header              = "data.sfgov.org"
  priority                        = 1
  weight                         = 500
}
...
resource "azurerm_cdn_frontdoor_route" "geofoodtruck_az_cdn_frontdoor_app_store_route" {
  ...
  cache {
    ...
    content_types_to_compress     = ["text/html", "text/javascript", "application/javascript", "text/css", "image/png", "image/jpeg", "application/json", "text/plain"]
  }
}
